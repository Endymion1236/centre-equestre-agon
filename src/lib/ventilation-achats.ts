...
import { compteBanque, compteFournisseur, ventilerDepense } from "./plan-comptable-achats";

export function comptesProposes(ligne: LigneMois) {
  const imputation = ventilerDepense(ligne);
  const banque = compteBanque(ligne.compte);
  ...
  const controles = [
    ...
    ...(!banque.compte ? ["Compte bancaire à identifier."] : []),
...
export function construireExportVentilationAchats(lignes: LigneMois[]) {
  const entetes = ["Identifiant opération", "Mois", "Date opération", "Fournisseur / libellé", "Catégorie", "Montant débité EUR", "Compte d’imputation proposé", "Libellé compte", "Origine proposition", "Compte fournisseur reconnu", "Compte bancaire source", "Compte banque proposé", "État", "Points à vérifier", "Identifiant pièce", "N° facture"];
