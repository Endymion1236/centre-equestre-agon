/**
 * src/lib/compta-synthese-pdf.ts — le PDF de synthèse comptable d'un mois.
 *
 * Vivait dans la route /api/compta-export-pdf, donc appelable seulement par
 * le navigateur. L'envoi mensuel à la comptable (lib/envoi-comptable) a
 * besoin du même document côté serveur, sans requête : la génération est
 * ici, la route ne fait plus que l'authentification et la réponse HTTP.
 *
 * Contenu :
...
export interface DonneesSynthesePdf {
  /** "AAAA-MM" */
  period: string;
  payments: any[];
  encaissements: any[];
...
export async function genererPdfSyntheseCompta({ period, payments, encaissements }: DonneesSynthesePdf): Promise<Buffer> {
    // ─── Calculs globaux ───
