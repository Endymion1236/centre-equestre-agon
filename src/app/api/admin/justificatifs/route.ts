...
import { verifierReglementGroupe, type PieceDuGroupe } from "@/lib/reglement-groupe";
  ...
  try {
    ...
    if (body.action === "classer-paie") {
      await adminDb.runTransaction(async tx => {
        ...
        tx.update(ref, { paieValidee: true, autoBloque: true, decisionHumaine: true, controleManuel: true, reviewedBy: auth.uid, reviewedAt: FieldValue.serverTimestamp() });
    ...
    if (body.action === "manuel") {
      await adminDb.runTransaction(async tx => {
        ...
        // « Je m'en occupe moi-même » : c'est bien une décision d'association,
        // elle sort la pièce du rapprochement automatique.
        tx.update(ref, { autoBloque: true, decisionHumaine: true, controleManuel: true });
    ...
    if (body.action === "corriger") {
      ...
      await adminDb.runTransaction(async tx => {
        ...
        // Corriger une lecture prépare le rapprochement, elle ne le refuse
        // pas : on trace la correction (`lectureCorrigee`) sans poser la
        // décision d'association qui écarterait la pièce de l'automatique.
        tx.update(ref, { extraction, paieValidee: false, lectureCorrigee: true, reviewedBy: auth.uid, reviewedAt: FieldValue.serverTimestamp() });
    ...
    // ── Règlement groupé : plusieurs factures pour un seul débit ─────────
    //
