...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── 1. Créneaux disponibles (logique partagée : lib/dispo.ts) ──────
    //    Même calcul que l'agent admin et, à terme, l'agent téléphonique.
    const { horizon, autresDispo, activitesDispo, stagesDispo, stageGroupMap, creneauMap } =
      await calculerDisponibilites(adminDb, { today });
