/**
 * Cron — questionnaire de satisfaction post-stage.
 *
 * Chaque matin, repère les stages MULTIJOURS terminés la veille, et pour chaque
 * enfant inscrit crée une invitation (avec ses vrais moniteurs, lus depuis les
 * créneaux où il était inscrit) puis envoie le mail à la famille via Resend.
 *
 * Auth : `Authorization: Bearer <CRON_SECRET>` (cron Vercel) OU admin connecté.
 * Paramètres :
 *   ?date=YYYY-MM-DD  -> traite les stages terminés ce jour-là (défaut: hier, Paris)
 *   ?dry=1            -> prévisualisation : ne crée rien, n'envoie rien
 *   ?to=email         -> redirige TOUS les mails vers cette adresse (test)
 *
 * Idempotent : une invitation par (stage, enfant) ; pas de renvoi si déjà créée.
 * Sécurité d'envoi : passe par le garde-fou email-guard (mode restreint par défaut).
 */
...
async function handle(req: NextRequest) {
  // ── Auth : secret cron OU admin connecté ─────────────────────────────────
  const authHeader = req.headers.get("authorization") || "";
  let authorized = !!process.env.CRON_SECRET && authHeader === `Bearer ${process.env.CRON_SECRET}`;
  if (!authorized) {
  ...
  const dateFin = req.nextUrl.searchParams.get("date") || parisDate(new Date(Date.now() - 86400000));
  const dry = req.nextUrl.searchParams.get("dry") === "1";
  const toOverride = (req.nextUrl.searchParams.get("to") || "").trim();
  ...
  for (const g of groups.values()) {
    ...
    if (dates[dates.length - 1] !== dateFin) continue; // terminés la veille (= dateFin)

    const semaine = lundiDe(dateFin);
    const stageKey = `${norm(g.title)}_${dates[0]}`;

    // Par enfant : moniteurs des créneaux (de ce stage) où il est inscrit
    ...
    // Idempotence : enfants déjà invités pour ce stage
