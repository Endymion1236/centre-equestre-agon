...
function StageCard({ title, subtitle, price, features, unite = "/ semaine", highlight = false }: { title: string; subtitle: string; price: string; features: string[]; unite?: string; highlight?: boolean }) {
  return (
    <article className={`relative flex h-full flex-col overflow-hidden rounded-[24px] border p-6 transition-all hover:-translate-y-1 ${highlight ? "border-blue-700 bg-[linear-gradient(145deg,#07111f,#12346b)] text-white shadow-[0_22px_55px_rgba(12,26,46,0.16)]" : "border-blue-500/[0.08] bg-white shadow-[0_12px_38px_rgba(12,26,46,0.04)]"}`}>
      {highlight && <div className="absolute right-4 top-4 rounded-full bg-gold-400 px-3 py-1 font-body text-[11px] font-bold uppercase tracking-wide text-blue-950">Le plus choisi</div>}
      <div className={`font-body text-[11px] font-bold uppercase tracking-[0.15em] ${highlight ? "text-gold-300" : "text-gold-600"}`}>{subtitle}</div>
    ...
    <>
      <section className="bg-cream px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-[1120px]">
          <div className="mb-10 text-center"><div className="font-body text-[11px] font-bold uppercase tracking-[0.18em] text-gold-500">Stages vacances</div><h2 className="mt-3 font-display text-3xl font-bold text-blue-950 sm:text-4xl">Une semaine complète à poney</h2><p className="mx-auto mt-4 max-w-2xl font-body text-base leading-relaxed text-slate-500">Les groupes sont organisés par âge et niveau. La réservation affiche les dates et horaires réellement disponibles.</p></div>
        ...
        <div className="mx-auto grid max-w-[1120px] gap-7 lg:grid-cols-[1fr_1fr] lg:items-stretch">
          ...
          <div className="flex flex-col overflow-hidden rounded-[28px] border border-blue-500/[0.08] bg-cream shadow-[0_18px_50px_rgba(12,26,46,0.06)]">
            <div className="border-b border-blue-500/[0.07] p-5 sm:p-6">
              <div className="font-body text-[11px] font-bold uppercase tracking-[0.16em] text-gold-500">Tarifs des promenades</div>
        ...
        <section className="bg-sand px-6 py-20">
          <div className="mx-auto grid max-w-[1120px] gap-7 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
            ...
            <div className="rounded-[28px] border border-blue-500/[0.08] bg-white p-5 shadow-[0_18px_50px_rgba(12,26,46,0.05)] sm:p-6">
              <div className="mb-5"><div className="font-body text-[11px] font-bold uppercase tracking-[0.16em] text-gold-500">Forfaits annuels</div><h2 className="mt-2 font-display text-2xl font-bold text-blue-950">Choisissez la fréquence qui convient</h2></div>
              <div className="grid gap-3">
                {annualCourses.map((course, index) => (
                  <div key={`${course.label}-${index}`} className="flex items-center justify-between gap-5 rounded-[20px] border border-blue-500/[0.07] bg-cream p-5">
                    ...
                    <div className="flex-shrink-0 text-right">{course.from ? <div className="font-body text-xs text-slate-400">à partir de</div> : null}<div className="font-display text-2xl font-bold text-blue-700">{course.price}€</div><div className="font-body text-xs text-slate-400">forfait annuel</div></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="bg-white px-6 py-20">
        <div className="mx-auto grid max-w-[1000px] gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
          <div><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gold-50 text-gold-600"><Trophy size={23} /></div><div className="mt-5 font-body text-[11px] font-bold uppercase tracking-[0.18em] text-gold-500">Challenges & concours</div><h2 className="mt-3 font-display text-3xl font-bold text-blue-950">Participer à la vie sportive du club</h2><p className="mt-4 font-body text-sm leading-relaxed text-slate-500">Les tarifs et niveaux sont précisés pour chaque épreuve. Les inscriptions apparaissent dans le planning et l’espace famille.</p></div>
      ...
      <section className="bg-cream px-6 pb-24 pt-8">
        <div className="mx-auto grid max-w-[1000px] overflow-hidden rounded-[28px] bg-[linear-gradient(135deg,#07111f,#12346b)] text-white shadow-[0_22px_65px_rgba(12,26,46,0.14)] md:grid-cols-[1fr_auto] md:items-center">
          <div className="p-7 sm:p-9"><div className="flex items-center gap-2 font-body text-[11px] font-bold uppercase tracking-[0.15em] text-gold-300"><CreditCard size={15} /> Paiement</div><h2 className="mt-3 font-display text-2xl font-bold text-white">Réservez en ligne, puis suivez vos règlements dans l’espace famille</h2>{tariffs.paiement_note && <p className="mt-3 max-w-2xl font-body text-sm leading-relaxed text-white/55">{tariffs.paiement_note}</p>}</div>
