...
function EnrollPanel({ creneau, families, allCreneaux, payments, allCartes, allForfaits, onClose, onEnroll, onUnenroll, onRefresh }: {
  ...
  // Les familles créées depuis cet écran, le temps de la session : elles ne
  // sont pas encore dans `families`, qui vient du planning.
  const [localFamilies, setLocalFamilies] = useState<any[]>([]);
  ...
  const [rechargerAttente, setRechargerAttente] = useState(0);
  ...
  // Échelle tarifaire partagée avec l'espace famille, demi-fréquences comprises
  // (une quinzaine vaut 0,5×/semaine). Voir src/lib/forfait-pricing.ts.
  ...
  //
  // Un forfait en quinzaine ne compte que pour MOITIÉ : le cavalier en garde
  ...
  const placeRestante = Math.max(0, 3 - frequenceDejaInscrite);
  const freqMaxAjoutable = Math.floor(placeRestante / coefRythme);
  // Vrai quand il reste de la place, mais pas assez pour un créneau au rythme
  // choisi : typiquement 0,5 restant alors qu'on demande un créneau hebdo.
  // Le message doit dire quoi faire, pas seulement « c'est plein ».
  const placeInsuffisantePourLeRythme = freqMaxAjoutable === 0 && placeRestante > 0;
  // Le plafond de 3×/semaine doit bloquer l'inscription, pas seulement masquer
  // les boutons : `frequenceCours` garde sa valeur quand on change de cavalier
  // dans le même panneau, et on pouvait valider une fréquence devenue
  // impossible pour le nouvel enfant.
  const frequenceHorsPlafond = inscriptionMode === "annuel"
    && (freqMaxAjoutable === 0 || frequenceCours > freqMaxAjoutable);
  ...
  // Ce que ce forfait ajoute, en fréquence hebdomadaire équivalente : une
  // quinzaine 1×/sem ajoute 0,5, pas 1. C'est ce qui remplace l'ancien
  // « coefficient 0,5 appliqué au prix » — la moitié est désormais portée par
  // la fréquence elle-même, donc le cumul de deux quinzaines retombe bien sur
  ...
  // L'acompte suit la même règle qu'en ligne (panier-reservation) : un montant
  // fixe par enfant, jamais plus que le total. Cet écran la recalculait pour
  // son compte ; une famille ne peut pas lire un chiffre en ligne et en voir
  // un autre sur sa facture.
  const showAcompte = stageMode === "semaine" && stagePayChoice === "acompte"
    && acompteApplicable(stageLines.length, stageTotalTTC);
  const { acompte: acompteDuStage, solde: soldeDuStage } =
    montantsAcompteStage(stageLines.length, stageTotalTTC);
  const stageAcompte = showAcompte ? acompteDuStage : stageTotalTTC;
  const stageSolde = showAcompte ? soldeDuStage : 0;
  ...
  const handleEnroll = async () => {
    await inscrireDepuisPanneau({
      children, panelToast, acompteMode, acompteRef, acompteReglement,
      adhesion, ajoutHeureAdmin, allCreneaux, annualPayMode, assuranceOccasionnelle,
      checkAndAlertIfFull, compCoaching, compEpreuves, conversion,
      creneau, dateFinSaisonEffective, extraSlots, fam, familyDiscountAmount,
      familyDiscountPercent, freeEnroll, freeReason, freqCumuleeAdmin, frequenceCours,
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-start md:items-center justify-center p-3 pb-6 md:p-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92dvh] md:max-h-[88vh] my-auto flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 border-b border-blue-500/8" style={{ borderLeftWidth: 4, borderLeftColor: color }}>
          ...
          <PanneauPedagogie creneau={creneau as any} />
        </div>
        ...
        <div ref={inscritsRef} className="p-5">
          ...
          <FormulaireEmailCreneau
            creneau={creneau} enrolled={enrolled} families={families}
            panelToast={panelToast} ouvert={showEmailForm}
          ...
          {/* ── Liste d'attente et ajout manuel quand la séance est pleine ── */}
          <PanneauListeAttente
            creneau={creneau} allCreneaux={allCreneaux} allFamilies={allFamilies}
            spots={spots} enrolled={enrolled}
            selFam={selFam} setSelFam={setSelFam} selChild={selChild} setSelChild={setSelChild}
            search={search} setSearch={setSearch} panelToast={panelToast}
          ...
          {spots > 0 && (creneau as any).status !== "closed" && (<div className="border-t border-blue-500/8 pt-4"><h3 className="font-body text-sm font-semibold text-blue-800 mb-3"><UserPlus size={16} className="inline mr-1"/>Inscrire</h3><div className="flex flex-col gap-3">
            ...
            {showNewFamily && (
              <FormulaireNouvelleFamille
                panelToast={panelToast}
                onFermer={() => setShowNewFamily(false)}
                onCreee={(famille, premierCavalierId) => {
                  setLocalFamilies(prev => [...prev, famille as any]);
                  setSelFam(famille.firestoreId);
                  setSelChild(premierCavalierId);
                  setShowNewFamily(false);
                  setSearch(famille.parentName || "");
                }} />
            )}
            {fam && !isStage && (
              <div>
                      ...
                      {ajoutHeureAdmin && (
                        <div className="mb-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                          <p className="font-body text-[11px] text-green-700">
                            🏇 Déjà <strong>{formatFrequence(frequenceDejaInscrite)}×/sem</strong> cette saison — heures ajoutées au <strong>tarif dégressif</strong> (différentiel), pas à plein tarif.
                          </p>
                        </div>
                      )}
                      {freqMaxAjoutable === 0 ? (
                        <div className="p-2 bg-amber-50 border border-amber-200 rounded-lg font-body text-xs text-amber-800">
                          {placeInsuffisantePourLeRythme
                            ? <>Il ne reste que <strong>{formatFrequence(placeRestante)}×/sem</strong> sous le plafond de 3×/sem. Cochez « une semaine sur deux » ci-dessous pour ajouter un créneau à mi-temps.</>
                            : <>Déjà au maximum (3×/sem) cette saison.</>}
                        </div>
                      ) : (
                        <div className="flex gap-2">
                          {([1, 2, 3] as const).filter(f => f <= freqMaxAjoutable).map(f => {
                            // Même échelle que le calcul réel : la quinzaine
                            // divise la fréquence ajoutée, pas le prix final.
                            const fEquiv = f * coefRythme;
                            const prixF = Math.round(ajoutHeureAdmin
                              ? Math.max(0, prixForfaitPlein(Math.min(3, frequenceDejaInscrite + fEquiv)) - prixForfaitPlein(frequenceDejaInscrite))
                              : prixForfaitPlein(fEquiv));
                            return (
