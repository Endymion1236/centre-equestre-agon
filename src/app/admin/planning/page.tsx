...
function SimpleCreneauForm({ activities, onSave, onCancel, defaultDate }: { activities: Activity[]; onSave: (c: Partial<Creneau>[]) => Promise<void>; onCancel: () => void; defaultDate?: string; }) {
  ...
  const sub = async () => { if (!actId || !act) return; setSaving(true); const ttc = (act as any).priceTTC || (act.priceHT || 0) * (1 + (act.tvaTaux || 5.5) / 100); await onSave([{ activityId: actId, activityTitle: act.title, activityType: act.type, date, startTime: st, endTime: et, monitor: mon, maxPlaces: mp, enrolledCount: 0, enrolled: [], status: "planned", priceHT: ttc / (1 + (act.tvaTaux || 5.5) / 100), priceTTC: ttc, tvaTaux: act.tvaTaux || 5.5 }]); setSaving(false); };
  ...
  return (
    <Card padding="md" className="mb-6 border-blue-500/15">
      <div className="flex justify-between items-center mb-4"><h3 className="font-body text-base font-semibold text-blue-800">Créneau unique</h3><button onClick={onCancel} className="text-gray-400 bg-transparent border-none cursor-pointer"><X size={20}/></button></div>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2"><input type="date" value={date} onChange={e => setDate(e.target.value)} className={`${inp} flex-1`}/><input type="time" value={st} onChange={e => setSt(e.target.value)} className={`${inp} w-24`}/><input type="time" value={et} onChange={e => setEt(e.target.value)} className={`${inp} w-24`}/></div>
        ...
        <button onClick={sub} disabled={!actId || saving} className={`flex items-center justify-center gap-2 py-2.5 rounded-lg font-body text-sm font-semibold border-none cursor-pointer ${!actId||saving?"bg-gray-200 text-gray-400":"bg-blue-500 text-white hover:bg-blue-400"}`}>{saving?<Loader2 size={16} className="animate-spin"/>:<Check size={16}/>} Créer</button>
      </div>
    </Card>
  );
}
  ...
  return (
    <div>
      ...
      {viewMode==="day"&&<>
        ...
        {loading?<div className="text-center py-16"><Loader2 className="w-8 h-8 animate-spin text-blue-500 mx-auto"/></div>:
        dayCreneaux.length===0?<Card padding="lg" className="text-center"><span className="text-4xl block mb-3">📅</span><p className="font-body text-sm text-gray-500">Aucun créneau.</p></Card>:
