          ...
          <Card key={c.id} padding="md" className="cursor-pointer hover:shadow-lg" hover>
            <div onClick={()=>setSelectedCreneau(c)}>
              ...
              {en.length>0&&<div className="ml-[68px] flex flex-wrap gap-2">{en.map((e:any)=>{
                ...
                const hasPaid = isCard || isForfaitPaid || payments.some((p: any) => p.familyId === e.familyId && p.status === "paid" && (p.items||[]).some(matchesThis));
                const hasPending = !hasPaid && !isCard && (isForfaitPending || payments.some((p: any) => p.familyId === e.familyId && (p.status === "pending" || p.status === "partial") && (p.items||[]).some(matchesThis)));
                const statusColor = isCard ? "#2050A0" : isForfaitPaid ? "#059669" : isForfaitPending ? "#d97706" : hasPaid ? "#16a34a" : hasPending ? "#d97706" : "#9ca3af";
