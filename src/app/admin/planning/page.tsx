...
import { Creneau, EnrolledChild, typeColors, dayNames, dayNamesFull, payModes, getWeekDates, fmtDate, fmtDateFR, fmtMonthFR, compareCreneaux, itemMatchesCreneau } from "./types";
          ...
          <Card key={c.id} padding="md" className="cursor-pointer hover:shadow-lg" hover>
            <div onClick={()=>setSelectedCreneau(c)}>
              ...
              {en.length>0&&<div className="ml-[68px] flex flex-wrap gap-2">{en.map((e:any)=>{
                ...
                const hasPaid = isCard || isForfait || payments.some((p: any) => p.familyId === e.familyId && p.status === "paid" && (p.items||[]).some(matchesThis));
                const hasPending = !hasPaid && !isCard && !isForfait && payments.some((p: any) => p.familyId === e.familyId && (p.status === "pending" || p.status === "partial") && (p.items||[]).some(matchesThis));
                const statusColor = isCard ? "#2050A0" : isForfait ? "#059669" : hasPaid ? "#16a34a" : hasPending ? "#d97706" : "#9ca3af";
                const statusBg   = isCard ? "#EDF2FA"  : isForfait ? "#ecfdf5" : hasPaid ? "#f0fdf4"  : hasPending ? "#fffbeb"  : "#f3f4f6";
                const statusIcon = isCard ? "🎟️" : isForfait ? "📅" : hasPaid ? "✓" : hasPending ? "…" : "—";
                const statusLabel = isCard ? "carte" : isForfait ? "forfait" : hasPaid ? "réglé" : hasPending ? "en attente" : "non réglé";
