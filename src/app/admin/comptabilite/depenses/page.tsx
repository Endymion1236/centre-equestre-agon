...
import { etatPiece, resteAFaire, sansFactureParNature, sansTvaParNature } from "@/lib/piece-attendue";
import { resumeGroupe, verifierReglementGroupe } from "@/lib/reglement-groupe";
...
export default function DepensesPage() {
  ...
  // Masquer ce qui n'attend aucune facture (commissions, emprunts, salaires,
  // compte FFE…) : un choix d'affichage, retenu sur cet appareil.
  const [masquerFrais, setMasquerFrais] = useState(false);
  useEffect(() => { try { if (localStorage.getItem("depenses.masquerSansFacture") === "1" || localStorage.getItem("depenses.masquerFrais") === "1") setMasquerFrais(true); } catch { /* stockage indisponible */ } }, []);
  const basculerMasquerFrais = (v: boolean) => { setMasquerFrais(v); try { localStorage.setItem("depenses.masquerSansFacture", v ? "1" : "0"); localStorage.removeItem("depenses.masquerFrais"); } catch { /* stockage indisponible */ } };
   ...
   *
   * Avec plusieurs centaines de pièces, dérouler la liste pour retrouver
   * « Padd - 199,98 € - 2026-07-07.pdf » prend plus de temps que le
   * rapprochement lui-même. Comme les fichiers portent leur montant dans leur
   * nom, une recherche sur ce texte suffit — et le montant du débit, qu'on
   * connaît déjà, en est la valeur de départ la plus utile.
   */
  const [rechercheePiece, setRecherchePiece] = useState("");
  /** Factures retenues pour un règlement groupé (plusieurs factures, un débit). */
  ...
  const panneau = ligne && <section id="association-ligne" className="rounded border bg-blue-50 p-4 space-y-3">
        ...
        <label className="block">Type de rattachement <select className="border rounded p-2" disabled={busy} value={mode} onChange={e => { setMode(e.target.value); setCorrection(null); }}><option value="normal">Facture ou bulletin — paiement unique</option><option value="echeance">Échéance d’une facture</option><option value="groupe">Règlement groupé — plusieurs factures</option><option value="per">Attestation PER — contrôle fiscal</option><option value="ffe">Relevé du compte FFE — avance licences / engagements</option></select></label>
        {mode === "groupe" && (() => {
          // Composition d'un règlement groupé : on coche les factures et le
          // total s'affiche en direct. Le bouton ne s'active que lorsque la
          // somme tombe exactement sur le débit — c'est ce contrôle qui évite
        ...
        {mode !== "groupe" && (() => {
          const disponibles = pieces.filter(p => !p.retire && (!p.depenseId || p.depenseId === ligne.id || p.modeRattachement === mode && !!p.paiementsAssocies?.length));
          // La recherche porte sur le nom du fichier ET sur le montant lu :
          ...
          return <>
            <label className="block text-sm">Rechercher une pièce (montant, fournisseur, nom du fichier)
              <input className="mt-1 block w-full rounded border p-2" value={rechercheePiece} disabled={busy}
                onChange={e => setRecherchePiece(e.target.value)} placeholder="199,98" aria-label="Rechercher une pièce" />
            </label>
            <p className="text-xs text-slate-600">{filtrees.length} pièce(s) sur {disponibles.length}{q ? ` pour « ${rechercheePiece.trim()} »` : ""}.{q && !filtrees.length ? " Videz la recherche pour voir toute la liste." : ""}</p>
            <select aria-label="Pièce existante" className="min-w-0 w-full max-w-full border rounded p-2" value={choix?.id || ""} disabled={busy} onChange={e => { setChoix(pieces.find(p => p.id === e.target.value) || null); setCorrection(null); }}><option value="">Choisir une pièce déjà importée</option>{choix && !filtrees.some(p => p.id === choix.id) && <option value={choix.id}>{choix.nom} — pièce ouverte</option>}{filtrees.map(p => <option key={p.id} value={p.id}>{p.nom}</option>)}</select>
          </>;
        })()}
        {choix && <><p>{choix.nom}</p>
  ...
  return <main className="depenses-page min-w-0 w-full space-y-5">
    ...
    {vue === "pieces" ? <><p>Documents en attente et archives. Pour associer un paiement, revenez au tableau des opérations.</p><PiecesSansLigne /></> : vue === "synthese" ? <VueParPoste /> : <>
      ...
      <div className="flex flex-wrap gap-3"><label>Mois <input type="month" className="border rounded p-2" disabled={busy} value={mois} onChange={e => { if(e.target.value) { setMois(e.target.value); setCible(null); setChoix(null); } }} /></label>
        ...
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={masquerFrais} onChange={e => basculerMasquerFrais(e.target.checked)} /> <span title="Commissions et frais bancaires, emprunts et intérêts, assurances et impôts prélevés, compte FFE, salaires et cotisations, virements internes, dépenses personnelles">Masquer ce qui n’attend aucune facture</span>{sansFacture.length ? ` (${sansFacture.length} · ${eurosCourt(sansFacture.reduce((t, l) => t + (l.montant || 0), 0))})` : ""}</label>
  ...
  : l.poste === POSTE_ASSURANCES || l.poste === POSTE_IMPOTS
    ? `Je confirme que ce prélèvement de ${euros(l.montant)} (${l.poste === POSTE_ASSURANCES ? "assurance" : "impôt ou taxe"}, sans TVA) figure sur le relevé ${l.note ? "indiqué" : `du compte ${l.compte || "bancaire"} de ce mois`}, que je conserve pour la comptable avec ${l.poste === POSTE_ASSURANCES ? "l'avis d'échéance" : "l'avis d'imposition"}.`
    : `Je confirme que la commission de ${euros(l.montant)} figure sur le relevé ${l.note ? "indiqué" : `du compte ${l.compte || "bancaire"} de ce mois`} et que je conserve ce relevé original pour le comptable. Ce choix ne valide aucune TVA déductible.`)) void agir(endpoint, { action: "justifier-releve", id: l.id, confirme: true }); }}>{l.poste === CATEGORIE_EMPRUNTS ? "Échéance de prêt : relevé + tableau d'amortissement" : "Utiliser le relevé comme justificatif"}</button>}{l.poste === CATEGORIE_EMPRUNTS && <p className="text-xs">Échéance de prêt : capital hors charges, intérêts en charges financières. La comptable ventile d'après le tableau d'amortissement.</p>}</>}<label className="block underline cursor-pointer">Importer une pièce<input aria-label={`Importer un justificatif pour ${l.fournisseur}`} className="block min-w-0 w-full max-w-full" type="file" accept="application/pdf,image/jpeg,image/png" disabled={busy || !!l.rapprochementExclu || l.source !== "releve-bancaire"} onChange={e => { const f = e.target.files?.[0]; e.target.value = ""; if (f) void importer(l, f); }} /></label><button disabled={busy || !!l.rapprochementExclu || l.source !== "releve-bancaire"} className="underline" onClick={() => { setMessage(""); setMode("normal"); setCible(l.id); setChoix(null); setCorrection(null); setRecherchePiece(l.montant.toFixed(2).replace(".", ",")); setGroupe([]); }}>Choisir une pièce existante</button><button disabled={busy} className="block underline text-left" onClick={() => void diagnostiquer(l)}>Pourquoi le rapprochement automatique ne l&apos;a pas trouvée ?</button></>}</section>
        </div>
