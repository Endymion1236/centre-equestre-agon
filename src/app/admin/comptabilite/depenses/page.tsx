...
export default function DepensesPage() {
  ...
  let blocage = "";
  if (ligne && choix && !choix.retire && extraction) {
    try {
      if (mode === "normal") verifierAssociationTableau(extraction as unknown as Record<string, unknown>, ligne);
      else if (mode === "echeance") verifierEcheance(extraction as unknown as Record<string, unknown>, ligne.montant, (choix.paiementsAssocies || []).filter(a => a.id !== ligne.id).reduce((s, a) => s + a.montant, 0));
  ...
  const panneau = ligne && <section id="association-ligne" className="rounded border bg-blue-50 p-4 space-y-3">
        ...
        {choix && <><p>{choix.nom}</p>
          {mode !== "normal" && !choix.retire && <button disabled={busy || !!correction || !!blocage || (mode === "echeance" && !extraction)} className="bg-blue-900 text-white rounded p-3" onClick={async () => {
          ...
          {choix.retire ? <><p>Cette pièce est archivée.</p><button disabled={busy} className="underline" onClick={async () => { if (await agir(justifs, { action: "restaurer", id: choix.id })) setChoix({ ...choix, retire: false }); }}>Restaurer cette pièce pour la réutiliser</button></> : !extraction ? <button disabled={busy} className="underline" onClick={async () => { await agir(justifs, { action: "analyser", id: choix.id }); const r = await authFetch(`${justifs}?piece=${choix.id}`); if(r.ok) {const d = await r.json(); setChoix(d.pieces[0]);} }}>Lire cette pièce</button> : <>
            ...
            {correction && <form className="flex flex-wrap gap-3" onSubmit={async e => { e.preventDefault(); setBusy(true); try { await post(justifs, { action: "corriger", id: choix.id, extraction: correction }); const r = await authFetch(`${justifs}?piece=${choix.id}`); const d = await r.json(); if (!r.ok || !d.pieces?.[0]) throw new Error(d.error || "Actualisez la pièce après correction."); setChoix(d.pieces[0]); setCorrection(null); await charger(); } catch(err) {setMessage(err instanceof Error ? err.message : "Erreur");} finally {setBusy(false);} }}>
            ...
            <button hidden={mode !== "normal"} disabled={busy || !!correction || !!blocage} className="bg-blue-900 text-white rounded p-3" onClick={async () => { if (!window.confirm(`Confirmer la pièce de ${montantPiece} ${extraction.devise || "devise inconnue"} pour le paiement ${ligne.fournisseur} de ${euros(ligne.montant)} ? Vérifiez le bénéficiaire et la période.`)) return; if (await agir(endpoint, { action: "associer", id: ligne.id, pieceId: choix.id, confirme: true, montantPiece, devise: extraction.devise, montantEUR: ligne.montant })) { setCible(null); setChoix(null); } }}>Confirmer l’association à cette ligne</button>
          </>}
