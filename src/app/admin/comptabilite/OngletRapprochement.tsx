 ...
 *
 * Onglet « Rapprochement » : importer le relevé bancaire, voir ligne par ligne
 * ce qui a été rapproché, et traiter à la main ce qui reste.
 *
 * L'écran s'ouvre sur deux alertes qui valent mieux qu'un long discours : le
 * taux de rapprochement, et la liste des virements attendus depuis plus de
 * 7 jours (une famille qui a promis un virement et ne l'a pas fait).
 *
 * Les deux boutons de maintenance méritent une explication, parce qu'ils
 * touchent la base :
 *  • « Resynchroniser » repose les drapeaux `reconciledByBank` / `pointee` /
 *    `paid` à partir des lignes bancaires actuellement matchées. À utiliser
 *    quand l'écran « à remettre » ne reflète plus la réalité.
 *  • « Nettoyer doublons » retire les entrées `matchedEncs` qui désignent un
 *    encaissement déjà revendiqué par une autre ligne. Ce sont des fantômes
 *    laissés par d'anciennes versions du matching : ils gonflent le compteur
 *    « à remettre » sans qu'on puisse rien y faire autrement.
 * Les deux demandent confirmation et affichent leur rapport avant d'écrire.
 */
...
import { saveBankLinesByMonth, syncVersementsEspeces } from "./sync-firestore";
import type { BankLine, CaDetailPreview, Payment } from "./types";

export function OngletRapprochement({
  payments, bankLines, setBankLines, encaissementsCompta, remises, fetchData,
  handleCSVImport, updateAndSaveBankLines,
  expandedBankLine, setExpandedBankLine,
  setShowManualMatch, setManualSearch,
  setShowCADetailModal, setCaDetailText, setCaDetailPreview,
  nbIgnores, analyserRapprochement, iaLoading, iaStats, iaAnalysis,
}: {
  payments: Payment[];
  bankLines: BankLine[];
  setBankLines: (v: BankLine[]) => void;
  ...
  expandedBankLine: number | null;
  setExpandedBankLine: (v: number | null) => void;
