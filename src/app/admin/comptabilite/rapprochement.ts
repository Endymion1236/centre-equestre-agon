...
/**
 * src/app/admin/comptabilite/rapprochement.ts
 *
 * Le cœur du rapprochement bancaire : pour chaque ligne du relevé, retrouver
 * les encaissements saisis dans l'application qui la composent.
 *
 * Pourquoi ce module existe : c'est la partie la plus délicate de la compta du
 * centre, et elle vivait au milieu d'un gestionnaire d'événement `onChange`
 * dans page.tsx. Or elle n'a besoin d'aucun état React — seulement des données
 * du mois (encaissements, factures, bordereaux) et de la période. En la
 * sortant, elle devient lisible, testable, et surtout : on voit d'un coup
 * d'œil dans quel ORDRE les stratégies sont essayées, ce qui est toute la
 * subtilité du sujet.
 *
 * RÈGLE FONDATRICE : un encaissement ne peut être consommé qu'UNE SEULE FOIS.
 * Deux virements de 50 € le même jour ne doivent pas se partager le même
 * encaissement de 50 €, sinon l'un des deux « disparaît » de la liste des
 * encaissements à remettre et l'écart devient introuvable. D'où les Set
 * `usedEncIds` / `usedRemiseSepaIds` / `usedPaymentIds` / `usedRemiseIds`, qui
 * sont retournés à l'appelant : il en a besoin pour marquer ensuite les
 * documents Firestore correspondants.
 *
 * Les stratégies sont ordonnées de la plus sûre à la plus fragile
 * (bordereau exact > nom + montant > montant seul). Tout ce qui ne repose que
 * sur le montant est marqué `uncertain: true` : l'écran affiche un ⚠️ et
 * l'admin va vérifier.
 */
...
/**
 * Cherche une combinaison d'encaissements dont la somme (en centimes) vaut
 * `targetCents` (± 2 centimes).
 *
 * Sert aux remises de chèques : si 7 chèques ont été saisis mais que la remise
 * n'en contient que 6, on retrouve lesquels. Programmation dynamique sur les
 * sommes atteignables, O(n × S). Bridée à 25 transactions et 100 000 sommes
 * intermédiaires — au-delà on renonce plutôt que de faire ramer le navigateur.
 *
 * Sert aussi aux remises CB terminal : la banque peut faire plusieurs remises
 * dans la même journée, ou une transaction a pu être refusée. Exemple vécu :
 * 18 CB saisis = 2802 €, la banque remet 2766 € (une transaction refusée) →
 * on retrouve la combinaison de 17 CB qui fait 2766 €.
 */
export function findSubsetSum(encs: any[], targetCents: number): any[] | null {
...
/**
 * Parse la date d'une ligne bancaire.
 * Formats acceptés : DD/MM/YYYY, D/M/YYYY, YYYY-MM-DD, DD-MM-YYYY.
 */
...
/** Réduit un encaissement à ce qu'on affiche sous une ligne bancaire. */
export function encToDetail(e: any) {
  return {
...
/**
 * Rapproche chaque ligne du relevé avec les encaissements du centre.
 *
 * Les Set retournés disent ce qui a été consommé : `usedEncIds` sert à poser
 * `reconciledByBank` sur les encaissements, `usedRemiseIds` à pointer les
 * bordereaux. Sans eux, l'appelant devrait redeviner l'association et se
 * tromperait sur les doublons légitimes (trois promenades à 25 € le même jour
 * pour la même famille).
 */
export function rapprocherLignesBancaires(params: {
  parsed: BankLine[];
  period: string;
  ...
  remises: any[];
  remisesSepa: any[];
}): { matched: BankLine[]; usedEncIds: Set<string>; usedRemiseIds: Set<string> } {
  const { parsed, period, encaissementsCompta, payments, remises, remisesSepa } = params;
  ...
  return { matched, usedEncIds, usedRemiseIds };
}
