...
/**
 * src/app/admin/comptabilite/OngletRemises.tsx
 *
 * Onglet « Bordereaux remise » : préparer ce qu'on emmène physiquement à la
 * banque (chèques, espèces), imprimer le bordereau, puis le pointer une fois
 * l'argent crédité.
 *
 * TOUT LE TRAVAIL SE FAIT AU NIVEAU ENCAISSEMENT, jamais au niveau facture.
 * C'est la clé de cet écran : un paiement mixte (200 € = 150 € chèque + 50 €
 * espèces) donne deux encaissements de modes différents, qui partent à la
 * banque dans deux bordereaux différents, à deux dates différentes.
 *
 * Trois pièges historiques sont traités ici, et il ne faut pas les défaire :
 *  1. Les contre-passations : un encaissement annulé par un encaissement
 *     négatif de même montant s'auto-neutralise et ne doit plus apparaître
 *     dans « à remettre » — sinon on emporte à la banque un chèque qui n'existe
 *     plus.
 *  2. Les remises anciennes (« legacy ») qui ne référencent que des paymentIds
 *     et pas des encaissementIds : on ne les applique qu'au MODE de la remise,
 *     sinon remettre le chèque d'un paiement mixte faisait disparaître aussi
 *     sa partie espèces.
 *  3. Les encaissements déjà rapprochés directement par la banque
 *     (`reconciledByBank`, typiquement les CB) : ils ne passent pas par un
 *     bordereau physique.
 *
 * Pointer une remise ESPÈCES crée en plus une sortie dans le livre de caisse
 * (le fameux « versement banque »), avec le montant réellement crédité — qui
 * peut différer si la banque a refusé un billet.
 */
...
export function OngletRemises({
  remises, encaissementsCompta, payments, fetchData,
  remiseDateFrom, setRemiseDateFrom, remiseDateTo, setRemiseDateTo,
  remiseModeFilter, setRemiseModeFilter,
  aRemettreDateFrom, setARemettreDateFrom, aRemettreDateTo, setARemettreDateTo,
  remiseModeView, setRemiseModeView,
  selectedForRemise, setSelectedForRemise,
  editingRemiseId, setEditingRemiseId, editingRemiseSearch, setEditingRemiseSearch,
  openRemiseId, setOpenRemiseId,
  pointageRemiseId, setPointageRemiseId,
  pointageNote, setPointageNote, pointageDate, setPointageDate,
  pointageMontantReel, setPointageMontantReel,
}: {
  remises: any[];
  encaissementsCompta: any[];
  payments: Payment[];
  fetchData: () => void;
  remiseDateFrom: string;
  setRemiseDateFrom: (v: string) => void;
  remiseDateTo: string;
  setRemiseDateTo: (v: string) => void;
  remiseModeFilter: string;
  setRemiseModeFilter: (v: string) => void;
  aRemettreDateFrom: string;
  setARemettreDateFrom: (v: string) => void;
  aRemettreDateTo: string;
  setARemettreDateTo: (v: string) => void;
  remiseModeView: string;
  setRemiseModeView: (v: string) => void;
  selectedForRemise: Set<string>;
  setSelectedForRemise: React.Dispatch<React.SetStateAction<Set<string>>>;
  editingRemiseId: string | null;
  setEditingRemiseId: (v: string | null) => void;
  editingRemiseSearch: string;
  setEditingRemiseSearch: (v: string) => void;
  openRemiseId: string | null;
  setOpenRemiseId: (v: string | null) => void;
  pointageRemiseId: string | null;
  setPointageRemiseId: (v: string | null) => void;
  pointageNote: string;
  setPointageNote: (v: string) => void;
  pointageDate: string;
  setPointageDate: (v: string) => void;
  pointageMontantReel: string;
  setPointageMontantReel: (v: string) => void;
}) {
  // ──────────────────────────────────────────────────────────────────
