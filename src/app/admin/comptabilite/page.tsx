...
import TabRapprochementIgnores from "./TabRapprochementIgnores";
import TabRapprochement from "./TabRapprochement";
import TabRemise from "./TabRemise";
...
export default function ComptabilitePage() {
  ...
  const tabProps = { filteredPayments, totalHT, totalTVA, totalTTC, period, encaissementsCompta, byMode, tvaByRate, generateFEC, payments, aRemettreDateFrom, aRemettreDateTo, addDoc, collection, db, deleteDoc, doc, editingRemiseId, editingRemiseSearch, fetchData, getDocs, openRemiseId, pointageDate, pointageMontantReel, pointageNote, pointageRemiseId, query, remiseDateFrom, remiseDateTo, remiseModeFilter, remiseModeView, remises, selectedForRemise, serverTimestamp, setARemettreDateFrom, setARemettreDateTo, setEditingRemiseId, setEditingRemiseSearch, setOpenRemiseId, setPointageDate, setPointageMontantReel, setPointageNote, setPointageRemiseId, setRemiseDateFrom, setRemiseDateTo, setRemiseModeFilter, setRemiseModeView, setSelectedForRemise, updateDoc, where, analyserRapprochement, bankLines, expandedBankLine, getDoc, handleCSVImport, iaAnalysis, iaLoading, iaStats, nbIgnores, saveBankLinesByMonth, setBankLines, setCaDetailPreview, setCaDetailText, setExpandedBankLine, setManualSearch, setShowCADetailModal, setShowManualMatch, syncVersementsEspeces, updateAndSaveBankLines };
  ...
  return (
    <div>
      ...
      {!loading && tab === "remise" && <TabRemise {...tabProps} />}

      {/* ─── Rapprochement bancaire ─── */}
      {!loading && tab === "rapprochement" && <TabRapprochement {...tabProps} />}
      ...
      {!loading && tab === "rapprochement_ignores" && <TabRapprochementIgnores {...tabProps} />}

      {/* ─── Export FEC ─── */}
      {!loading && tab === "fec" && <TabFEC {...tabProps} />}

      {/* ─── Export CSV paramétrable ─── */}
      {!loading && tab === "export" && <TabExport {...tabProps} />}
