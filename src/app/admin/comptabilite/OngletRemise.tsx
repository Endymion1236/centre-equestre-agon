...
/**
 * src/app/admin/comptabilite/OngletRemise.tsx
 *
 * Les bordereaux de remise en banque : la liste des encaissements pas encore
 * remis, la constitution d'un bordereau, son impression, son pointage et son
 * historique.
 *
 * Cet onglet portait quatorze états — filtres, sélection, édition, pointage —
 * qui vivaient dans l'écran comptable alors qu'aucun autre onglet ne les lit.
 * Ils sont désormais ici, avec le rendu qu'ils commandent.
 *
 * Les modes qui ne passent pas par un bordereau (virement, prélèvement SEPA,
 * CB en ligne, avoir) restent exclus de la liste « à remettre » : la banque
 * les crédite sans qu'on dépose quoi que ce soit, ils se traitent au
 * rapprochement.
 */
...
export default function OngletRemise({ payments, remises, encaissementsCompta, fetchData }: OngletRemiseProps) {
  ...
  // Édition d'un bordereau (ajouter / retirer des encaissements)
  ...
  // Pointage manuel d'un bordereau
  ...
  const [pointageMontantReel, setPointageMontantReel] = useState("");
  const [openRemiseId, setOpenRemiseId] = useState<string | null>(null);
  // Sélection manuelle pour un nouveau bordereau
