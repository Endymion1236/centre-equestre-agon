...
function NoteField({ paymentId, initialNote, onSave }: { paymentId: string; initialNote: string; onSave: (note: string) => void }) {
  ...
  return (
    <div>
      ...
      {tab === "offerts" && (() => {
        ...
        const totalValeur = freePayments.reduce((s, p) => {
        ...
        return (
          <div>
            ...
            {freePayments.length === 0 ? (
              <Card padding="lg" className="text-center">
                <div className="w-14 h-14 rounded-2xl bg-green-50 flex items-center justify-center mx-auto mb-3"><Gift size={28} className="text-green-300" /></div>
                      ...
                      .map(p => {
                        ...
                        return (
                          <div key={p.id} className="px-5 py-3 border-b border-blue-500/8 last:border-b-0 flex items-center hover:bg-green-50/30">
                            ...
                            <span className="w-40 font-body text-xs text-slate-600 truncate">{items.map((i: any) => `${i.activityTitle}${i.childName ? ` — ${i.childName}` : ""}`).join(", ")}</span>
                            <span className="w-24"><Badge color="green">{(p as any).freeReason || "Offert"}</Badge></span>
