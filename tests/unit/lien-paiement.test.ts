 ...
 *
 *   1. un lien a un état lisible : valable 2 h, puis expiré ; annulé ; réglé ;
...
// ── 1. État d'un lien ─────────────────────────────────────────────────────
{
  ...
  assert("un lien vit 2 heures", DUREE_VALIDITE_LIEN_MS === 2 * 60 * 60 * 1000);
  assert("juste envoyé : valable", etatLien(envoye, t0 + 60_000) === "valide");
  assert("1 h 59 après : encore valable", etatLien(envoye, t0 + 119 * 60_000) === "valide");
  assert("2 h après : expiré", etatLien(envoye, t0 + DUREE_VALIDITE_LIEN_MS) === "expire");
