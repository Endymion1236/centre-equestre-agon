...
test("une adresse déjà prise par une autre fiche est signalée et reléguée", () => {
  const p = proposerAdresses(
    [src("libre@ex.fr", "compte"), src("prise@ex.fr", "commande")],
    ["PRISE@ex.fr "],
  );
  assert.equal(p[0].email, "libre@ex.fr", "même moins sûre, une adresse libre passe devant");
  assert.equal(p[1].dejaPrise, true);
  assert.equal(p[0].dejaPrise, false);
});
